...
TEST(test_algorithm_vector, vector_set) {
	...
	// 设置向量元素值
	int data1[] = { 1, 2, 3, 4 };
	alg::vector_set(v, data1, ARRAY_SIZE(data1));

	// 确认向量元素符合预期, 且未重建存储区
	EXPECT_EQ(v.size, ARRAY_SIZE(data1));
	EXPECT_EQ(v.capacity, DEFAULT_CAPACITY);
	EXPECT_TRUE(alg::is_int_array_eq(data1, v.array, v.size));

	// 再次设置向量元素值
	int data2[] = { 1, 2, 3, 4, 5, 6, 7, 8 };
	alg::vector_set(v, data2, ARRAY_SIZE(data2));

	// 确认向量元素符合预期, 且重建了存储区
	EXPECT_EQ(v.size, ARRAY_SIZE(data2));
	EXPECT_NE(v.capacity, DEFAULT_CAPACITY);
	EXPECT_TRUE(alg::is_int_array_eq(data2, v.array, v.size));
...
TEST(test_algorithm_vector, vector_add) {
	...
	EXPECT_EQ(alg::vector_add(v, 1), 1);
	EXPECT_EQ(alg::vector_add(v, 2), 2);
	EXPECT_EQ(alg::vector_add(v, 3), 3);
	EXPECT_EQ(alg::vector_add(v, 4), 4);
	...
	EXPECT_EQ(alg::vector_add(v, 5), 5);
	EXPECT_EQ(alg::vector_add(v, 6), 6);
	EXPECT_EQ(alg::vector_add(v, 7), 7);
	EXPECT_EQ(alg::vector_add(v, 8), 8);
...
TEST(test_algorithm_algorithm_vector, vector_append) {
	...
	alg::vector_append(v, data, ARRAY_SIZE(data));

	// 确认向量中包含的元素, 以及向量存储区未被重建
	EXPECT_EQ(v.capacity, DEFAULT_CAPACITY);
	EXPECT_EQ(v.size, ARRAY_SIZE(data));
	EXPECT_TRUE(alg::is_int_array_eq(data, v.array, ARRAY_SIZE(data)));

	alg::int_array_fill(data, ARRAY_SIZE(data), 5);
	...
	EXPECT_TRUE(alg::is_int_array_eq(data, v.array + 4, ARRAY_SIZE(data)));
