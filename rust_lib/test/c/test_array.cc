...
TEST(TEST_SUITE_NAME, c_array_sum) {
    ...
    int result = c_array_sum(arr, len);
    ASSERT_EQ(result, 15);
}
...
///
/// 返回的指针在 Rust 中是通过 `Vec` 类型实例创建的, 该指针需要调用 `c_free_array_by_vec` 函数, 在 Rust 中进行释放
...
/// @brief 测试调用 Rust 函数, 返回指向数组的指针
///
/// 返回的指针在 Rust 中是通过 `Box` 类型实例创建的, 该指针需要调用 `c_free_array_by_box` 函数, 在 Rust 中进行释放
